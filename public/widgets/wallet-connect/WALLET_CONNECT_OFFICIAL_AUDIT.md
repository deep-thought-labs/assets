...
- **Obtención del provider (EIP-6963 primero en SES/lockdown)**:
  - En entornos **SES/lockdown** (p. ej. LavaMoat), `window.ethereum` puede no recibir `chainChanged` ni devolver el `eth_chainId` real. El widget **prioriza el provider EIP-6963** (MetaMask por rdns/nombre, o el primero anunciado) cuando existe, y usa **window.ethereum** solo como fallback. Así se usa el canal que la extensión anuncia para la página y se reciben `chainChanged` y el chainId correcto.
  - Si no hay provider por EIP-6963, se usa `window.ethereum` (si es array: el que tenga `isMetaMask === true` o el primero). Ver `DETECCION_Y_CAMBIO_DE_RED_UNISWAP_VS_WIDGET.md`.
  - Además del listener **chainChanged**, el widget hace **polling** de `eth_chainId` cada 2,5 s cuando está conectado (y una primera comprobación a los 150 ms), para detectar cambio de red si el evento no se emite.
